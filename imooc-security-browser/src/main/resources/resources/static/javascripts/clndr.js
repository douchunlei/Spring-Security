...
;(function ( $, window, document, undefined ) {
  ...
  Clndr.prototype.createDaysObject = function(currentMonth) {
    ...
    // config the events list (if it exists) to events that are happening last month, this month and next month (within the current grid view)
    ...
    if(this.options.events.length) {
      ...
      // if we're using single-day events, use _clndrDateObject
        ...
        // config the adjacent months as well, if the option is true
        if(this.options.showAdjacentMonths) {
          var lastMonth = currentMonth.clone().subtract('months', 1);
          var nextMonth = currentMonth.clone().add('months', 1);
          this.eventsLastMonth = $(this.options.events).filter( function() {
      ...
      if(this.options.events) {
        ...
        if(this.options.multiDayEvents) {
          target.events = $.makeArray( $(this.options.events).filter( function() {
            // config the dates down to the ones that match.
